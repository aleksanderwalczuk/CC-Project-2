...
const startGame = (gameStarted, choosenMode) => {
  if(gameStarted === true) {
    ...
    const placeholder = elementFactory(
      'div',
...
const displayAnswer = (answers, correctAnswer, onAnsweredQuestion, callback) => {
  const sectionWrapper = document.querySelector('.section__wrapper');
  const placeholder = document.querySelector('.placeholder');

  sectionWrapper.removeChild(placeholder);
  sectionWrapper.appendChild(Answers(answers, correctAnswer, onAnsweredQuestion, callback));
}
